...
export const autoAnswers: {
    ...
    "саня привет": () => "прив",
    "дота чи не": (opitons) =>
        `@${opitons.tags["display-name"]} ` +
        (randomInteger(0, 1) == 0 ? "не" : "го"),
    "кто такой марк": (opitons) =>
        `@${opitons.tags["display-name"]} он ${randomMark()}`,
    пи: (opitons) => `@${opitons.tags["display-name"]} door`,
    "кто такой саня": (opitons) =>
        `@${opitons.tags["display-name"]} он красавчик`,
    "кто такая юля": (options) =>
        `@${options.tags["display-name"]} она ${randomJulia()}`,
    "кто такой коля": (options) => `@${options.tags["display-name"]} он лох`,
    "роль юли": (options) =>
        `@${options.tags["display-name"]} она ${randomRole()}`,
    "моя роль": (options) => {
        const role =
            options.tags["display-name"] === nicknames.me
                ? "красавчик"
                : randomRole();
